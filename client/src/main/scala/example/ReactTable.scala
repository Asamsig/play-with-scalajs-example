...
import chandu0101.scalajs.react.components.{DefaultSelect, ReactSearchBox}
...
object ReactTable {
  ...
  class Style extends StyleSheet.Inline {
    ...
    val sortIcon = styleF.bool(
      ascending =>
        styleS(&.after(fontSize(9 px), marginLeft(5 px), if (ascending) { content := "'\\25B2'" } else {
          content := "'\\25BC'"
        }))
    )
  ...
  def OptionRenderer[T, B](defaultValue: VdomNode = "",
                           bRenderer: CellRenderer[B])(fn: T => Option[B]): CellRenderer[T] =
  ...
  case class ColumnConfig[T](name: String,
                             ...
                             numeric: Boolean = false)(implicit val ordering: Ordering[T])

  def SimpleStringConfig[T](name: String,
                            stringRetriever: T => String,
                            width: Option[String] = None,
                            nowrap: Boolean = false,
                            numeric: Boolean = false): ReactTable.ColumnConfig[T] = {
    val renderer: CellRenderer[T] = if (nowrap) { t =>
      <.span(stringRetriever(t))
    } else { t =>
      stringRetriever(t)
    }
    ColumnConfig(name, renderer, width, nowrap, numeric)(ignoreCaseStringOrdering(stringRetriever))
  }
}
...
case class ReactTable[T](data: Seq[T],
  ...
  class Backend(t: BackendScope[Props, State]) {
      ...
      t.modState { state =>
        ...
        state.sortedState.get(columnIndex) match {
          case Some(SortDirection.asc) =>
            state.copy(filteredData = rows.sorted(ordering.reverse), sortedState = Map(columnIndex -> dsc), offset = 0)
          case _ =>
            state.copy(filteredData = rows.sorted(ordering), sortedState = Map(columnIndex -> asc), offset = 0)
        }
      }
    ...
    def render(props: Props, state: State): VdomElement = {
        ...
        <.tr(
          ...
          props.configs.zipWithIndex.map {
            case (config, columnIndex) =>
              ...
              cell(
                (^.cls := "mdl-data-table__cell--non-numeric").when(!config.numeric),
                ^.cursor := "pointer",
                ^.onClick --> sort(config.ordering, columnIndex),
                config.name.capitalize,
                props.style
                  .sortIcon(
                    state.sortedState.isDefinedAt(columnIndex) && state.sortedState(columnIndex) == SortDirection.asc
                  )
      ...
      def renderRow(model: T): TagMod =
        <.tr(
          ...
          props.configs
            .map(
              config =>
                <.td(^.whiteSpace.nowrap.when(config.nowrap), ^.verticalAlign.middle, config.cellRenderer(model), (^.cls := "mdl-data-table__cell--non-numeric").when(!config.numeric))
            )
            .toTagMod
        )
      ...
      <.div(
        ...
        ReactSearchBox(onTextChange = onTextChange(props) _).when(props.enableSearch),
        <.table(<.thead(renderHeader()), <.tbody(rows), props.style.table)
      )
    }
  }
  ...
  val component = ScalaComponent
    ...
    .componentWillReceiveProps(
      e =>
        Callback.when(e.currentProps.data != e.nextProps.data)(e.backend.onTextChange(e.nextProps)(e.state.filterText))
    )
    .build

  case class Props(data: Seq[T], configs: List[ColumnConfig[T]], rowsPerPage: Int, style: Style, enableSearch: Boolean)

  def apply() = component(Props(data, configs, rowsPerPage, style, enableSearch))
}
